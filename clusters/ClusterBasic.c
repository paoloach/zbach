...
#define LOCATION_DESCRIPTION_SIZE 17

uint8 locationDescription[LOCATION_DESCRIPTION_SIZE+1] = { 16, ' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ' };
...
void basicClusterWriteAttribute(ZclWriteAttribute_t * writeAttribute) {
	...
	case ATTRID_BASIC_LOCATION_DESC:
		if (writeAttribute->dataType == ZCL_DATATYPE_CHAR_STR){
			uint8 size =  writeAttribute->len > LOCATION_DESCRIPTION_SIZE ? LOCATION_DESCRIPTION_SIZE : writeAttribute->len;
			osal_memcpy((void *)&locationDescription, writeAttribute->dataPtr, size);
		}else
